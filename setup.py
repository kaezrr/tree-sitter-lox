...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_lox", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_lox": ["*.pyi", "py.typed"],
        "tree_sitter_lox.queries": ["*.scm"],
    },
    ext_package="tree_sitter_lox",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_lox/binding.c",
                "src/parser.c",
            ],
            define_macros=[
